...
void yundtu_report_message(void* buff, uint32_t* length)
{
    ...
    if ((p = strstr(buff, "FS@HTTP OK")) != NULL)
    {
        yundtu_debug("FS@HTTP OK\r\n");   // 如果接收到连接成功的消息
...
const at_command_handler_t at_command_init_table[] = {
   ...
   {"+++", "OK", 1000},  //进入AT指令模式	          																					
    {"AT+E=OFF\r\n", "OK", 1000},	 //关闭回显 																	
    {"AT+WKMOD1=HTTP\r\n", "OK", 1000},	  //设置通道一为HTTP													
    {"AT+HTPTP1=POST\r\n", "OK", 1000},		 //设置HTTP的请求方式为POST												
    {"AT+HTPURL1=http://httpbin.org/post\r\n", "OK", 1000},	//设置HTTP请求的域名/地址										
    {"AT+HTPHD1=\r\n", "OK", 1000},				 //	设置通道HTTP的HEAD信息												 
    {"AT+HTPPK1=3\r\n", "OK", 1000},  //设置HTTP返回信息过滤	
    {"AT+HTPTIM1=10\r\n", "OK", 1000},	 //设置与服务器响应超时时间														
    {"AT+HTPDT1=BODY\r\n", "OK", 1000},	 //设置与服务器响应超时时间		
 ...
 *      1: 所有参数都设置成功。
 */
int yundtu_HTTP_parameter_setting(void)
{
    int                                ret;
...
void yundtu_process(void)
{
    ...
    case YUNDTU_WRITE_CONFIG_PARAMETER:
        ret = yundtu_HTTP_parameter_setting();
        if (ret > 0)
        {
