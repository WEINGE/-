...
#define YUNDTU_PARAMETER_VERSION "2"   // YunDTU参数版本号
...
void yundtu_report_message(void* buff, uint32_t* length)
{
    ...
    if ((p = strstr(buff, "FS@MQTT CONNECTED")) != NULL)
    {
        yundtu_debug("FS@MQTT CONNECTED\r\n");   // 如果接收到连接成功的消息
        yundtu_link_ready_flag  = 1;            // 设置连接准备好标志
        yundtu_csq_interval_cnt = 0;            // 重置信号强度查询间隔计数
        yundtu_csq_err_cnt      = 0; 				// 重置信号强度查询错误计数
		yundtu_err_cnt = 0;
    }
    else if ((p = strstr(buff, "FS@MQTT CONNECT FAIL")) != NULL)
    {
        yundtu_debug("FS@MQTT CONNECT FAIL\r\n");   // 如果接收到连接失败的消息
		yundtu_err_cnt++;	
        yundtu_link_ready_flag = 0;                // 清除连接准备好标志
    }
    else if ((p = strstr(buff, "FS@MQTT DISCONNECT")) != NULL)
    {
        yundtu_debug("FS@MQTT DISCONNECT\r\n");   // 如果接收到断开连接的消
		yundtu_err_cnt++;	
        yundtu_link_ready_flag = 0;              // 清除连接准备好标志
    }
    if (p != NULL)
...
const at_command_handler_t at_command_init_table[] = {
    ...
    {"AT+E=OFF\r\n", "OK", 1000},																			//关闭回显
    {"AT+WKMOD=MQTT\r\n", "OK", 1000},															//MQTT模式
    {"AT+LINKDEBUG=ON\r\n", "OK", 1000},															//设置主动上报通道连接信息使能 
    {"AT+MQTTSV1=broker.emqx.io,1883\r\n", "OK", 1000},									//设置mqtt通道和服务参数
    {"AT+MQTTSSL1=OFF\r\n", "OK", 1000},															//设置通道的MQTT服务器连接参数 
    {"AT+MQTTCONN1=mqttx_wa465487ca,,,30,1\r\n", "OK", 1000},	//设置通道的MQTT服务器连接参数
    {"AT+MQTTMTPC1=OFF\r\n", "OK", 1000},														//设置通道的MQTT多主题使失能
    {"AT+MQTTSUB1=freertos_sub445,0\r\n", "OK", 1000},									//设置通道的MQTT订阅参数 
    {"AT+MQTTPUB1=freertos_pub445,0,0\r\n", "OK", 1000},								//设置通道的MQTT发布参数 
    {"AT+MQTTWLEN1=OFF\r\n", "OK", 1000},														//设置通道的MQTT遗嘱失能
    {"AT+MQTTWLDT1=will message\r\n", "OK", 1000},										//设置通道的MQTT遗嘱消息
 ...
 *      1: 所有参数都设置成功。
 */
int yundtu_MQTT_parameter_setting(void)
{
    int                                ret;
...
void yundtu_process(void)
{
    ...
    case YUNDTU_WRITE_CONFIG_PARAMETER:
        ret = yundtu_MQTT_parameter_setting();
        if (ret > 0)
        {
    ...
    case YUNDTU_TRANSMISSION:
        ...
        if (length > 0)
        {
            ...
            if (yundtu_link_ready_flag)
            {
			  ...
			  if (yundtu_err_cnt >= 10)															   // 如果错误计数达到10次，重置硬件
            {
                yundtu_err_cnt = 0;
                expression = YUNDTU_HW_RESET;
            }
