...
void yundtu_report_message(void* buff, uint32_t* length)
{
    ...
    if ((p = strstr(buff, "FS@UDP OK")) != NULL)
    {
        yundtu_debug("FS@UDP OK\r\n");   // 如果接收到连接成功的消息
        yundtu_link_ready_flag  = 1;            // 设置连接准备好标志
        yundtu_csq_interval_cnt = 0;            // 重置信号强度查询间隔计数
        yundtu_csq_err_cnt      = 0; 				// 重置信号强度查询错误计数
//		yundtu_err_cnt = 0;
    }
    else if ((p = strstr(buff, "FS@UDP CLOSE ")) != NULL)
    {
        yundtu_debug("FS@UDP CLOSE\r\n");   // 如果接收到连接失败的消息
...
const at_command_handler_t at_command_init_table[] = {
    ...
    {"AT+E=OFF\r\n", "OK", 1000},																		
    {"AT+WKMOD1=NET\r\n", "OK", 1000},														
    {"AT+SOCK1=UDP,112.125.89.8,47812\r\n", "OK", 1000},		//设置服务器地址												
    {"AT+HEARTEN1=OFF\r\n", "OK", 1000},									
 ...
 *      1: 所有参数都设置成功。
 */
int yundtu_UDP_parameter_setting(void)
{
    int                                ret;
...
void yundtu_process(void)
{
    ...
    case YUNDTU_WRITE_CONFIG_PARAMETER:
        ret = yundtu_UDP_parameter_setting();
        if (ret > 0)
        {
