...
class IARICFConfig():
    ...
    def make(self, file_path, add_folder):
        if add_folder:
...
def main():
    ...
    print(">>> handle 5513 icf")
    icf_5513 = IARICFConfig()
    if not icf_5513.get_config_args():
        return
    icf_5513.make(G_LDS_FILE_13, True)
    # if icf_5513.make(G_LDS_FILE_13):
    ...
    print(">>> handle 5515 icf and project")
    icf_5515 = IARICFConfig()
    if not icf_5515.get_config_args():
        return
    if icf_5515.make(G_LDS_FILE_15, False):
        gen = IARPrjGen(name[0])
        if not gen.copy_file(TEMPALTE_PRJ, "../IAR/"):
            return
        gen.parse_file(prj)
